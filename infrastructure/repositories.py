...
class SqlAlchemyProductRepository(ProductRepository):
    ...
    def update(self, product: Product):
        product_orm = self.session.query(ProductORM).filter_by(id=product.id).one_or_none() 
        if product_orm:
...
class SqlAlchemyOrderRepository(OrderRepository):
    ...
    def add(self, order:Order):
        ...
        for domain_item in order.items:
            product_orm_ref = self.session.query(ProductORM).filter_by(id=domain_item.product.id).one_or_none()
            if not product_orm_ref:
                self.logger.error(f"REPO: ProductORM with id {domain_item.product.id} not found for OrderItem.")
                raise ValueError(f"Product (ID: {domain_item.product.id}) referenced in order item not found in DB.")
        ...
        self.logger.info(f"REPO: Assigned order.id={order.id}. OrderORM has {len(order_orm.items)} items linked.")
        ...
        if order_orm:
            ...
            for item_orm in order_orm.items:
                if not item_orm.product:
                    self.logger.error(f"ProductORM not loaded for orderItemORM id {item_orm.id}")
                    continue
    ...
    def list(self) -> List[Order]:
        ...
        for order_orm in orders_orm:
            ...
            for item_orm in order_orm.items:
                if not item_orm.product:
                    self.logger.error(f"REPO: ProductORM not loaded for OrderItemORM id {item_orm.id} in order id {order_orm.id}")
                    continue
